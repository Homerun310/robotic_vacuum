...
// 장애물을 감지할 거리를 정의
#define OBSTACLE_THRESHOLD_FRONT_BOTTOM 220 // 장애물 감지 한계거리
#define OBSTACLE_THRESHOLD_LEFT_BOTTOM 220
#define OBSTACLE_THRESHOLD_RIGHT_BOTTOM 220
#define OBSTACLE_THRESHOLD_REAR_BOTTOM 220
...
bool isObstacleDetected() {
    ...
    if (frontBottomIRSensorValue < 220 ||
        leftBottomIRSensorValue < 220 ||
        rightBottomIRSensorValue < 220 ||
        rearBottomIRSensorValue < 220) {
        return true;
    }
    return false;
}
...
bool isRearDetected() {
  ...
  if (rearBottomIRSensorValue < 220) {
    return true;
  }
  return false;
}
...
bool isRightDetected() {
  ...
  if (rightBottomIRSensorValue < 220) {
    return true;
  }
  return false;
}
...
bool isLeftDetected() {
  ...
  if (leftBottomIRSensorValue < 220) {
    return true;
  }
  return false;
}
   ...
   if (op_state == BLT_MODE)
{
    ...
    {
        case 'F':
            if (frontState == false) moveForward();      
            else if (frontState == true) stopMotors();
            break;
        ...
        case 'B':
            if (rearState == false) moveBackward();      
            else if (rearState == true) stopMotors();
            break;
